...
export default function MyRentals() {
  ...
  return (
    <DashboardLayout>
      <h1 className="text-4xl font-display font-bold mb-8">My Rentals</h1>
      ...
      {isLoading ? (
        ...
        <Card className="p-12 text-center border-dashed">
          ...
          <p className="text-muted-foreground">When you rent tools, they will appear here.</p>
        </Card>
      ) : (
        <div className="space-y-4">
          {rentals.map(rental => (
              ...
              <CardContent className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                  ...
                  <div className="text-sm text-muted-foreground flex gap-4">
                    <span>Started: {format(new Date(rental.startedAt), 'MMM d, yyyy')}</span>
                    <span>Due: {format(new Date(rental.dueDate!), 'MMM d, yyyy')}</span>
                  </div>
                </div>
