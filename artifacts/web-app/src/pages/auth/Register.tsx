...
import { Hammer, Lock, Phone, User, Building2, ArrowLeft, ScrollText, ExternalLink, Check, Users } from "lucide-react";
...
type RoleType = "customer" | "shop_owner";
...
export default function Register() {
  ...
  const heroText = role === "customer"
    ? { title: "Asboblarni osongina ijara oling", sub: "O'zbekistondagi eng yaxshi qurilish asbob-uskunalari bir joyda. Tez, qulay va xavfsiz." }
    : { title: "Do'koningizni GetHelp.uz'ga qo'shing", sub: "Minglab qurilish kompaniyalari bilan birga ishlang. Asboblaringizni ijaraga bering va daromad oling." };
    ...
    <div className="min-h-screen flex bg-background">
      ...
      <div className="hidden lg:flex flex-1 relative bg-primary items-center justify-center overflow-hidden">
        ...
        <div className="z-10 text-primary-foreground p-12 max-w-lg">
          <motion.div key={role} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            <div className="w-16 h-16 bg-accent rounded-2xl flex items-center justify-center mb-8 shadow-2xl">
              {role === "customer" ? <Users size={32} className="text-white" /> : <Hammer size={32} className="text-white" />}
            </div>
            <h1 className="text-4xl font-display font-bold mb-4">{heroText.title}</h1>
            <p className="text-primary-foreground/80 text-lg leading-relaxed">{heroText.sub}</p>

            <div className="mt-10 space-y-3">
              {role === "customer" ? [
                "✅ Kunlik/haftalik/oylik ijara",
                "✅ Click, Payme, Paynet orqali to'lov",
                "✅ Loyallik ballari va referal bonus",
                "✅ Ijara tarixini kuzatib boring",
              ] : [
                "✅ QR kod boshqaruvi",
                ...
                <p key={f} className="text-primary-foreground/90 text-sm">{f}</p>
              ))}
            </div>
          </motion.div>
        </div>
      ...
      <div className="flex-1 flex items-start justify-center p-4 sm:p-8 overflow-y-auto">
        <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md py-6 sm:py-8">
          ...
          {/* Mobile logo */}
          <div className="lg:hidden w-10 h-10 bg-accent rounded-xl flex items-center justify-center mb-5 shadow-lg">
            <Hammer size={20} className="text-white" />
          </div>

          <h2 className="text-2xl sm:text-3xl font-display font-bold mb-2">Ro'yhatdan o'tish</h2>
          ...
          <div className="grid grid-cols-2 gap-2 mb-6 p-1 bg-muted rounded-xl">
            <button
              type="button"
              onClick={() => { setRole("customer"); setForm(f => ({ ...f, shopName: "" })); }}
              className={`flex items-center justify-center gap-2 py-2.5 px-3 rounded-lg font-semibold text-sm transition-all ${
                role === "customer" ? "bg-white dark:bg-card shadow text-foreground" : "text-muted-foreground hover:text-foreground"
              }`}
            >
              <Users size={16} className={role === "customer" ? "text-primary" : ""} />
              Mijoz
            </button>
            <button
              type="button"
              ...
              className={`flex items-center justify-center gap-2 py-2.5 px-3 rounded-lg font-semibold text-sm transition-all ${
                role === "shop_owner" ? "bg-white dark:bg-card shadow text-foreground" : "text-muted-foreground hover:text-foreground"
              }`}
            >
              <Building2 size={16} className={role === "shop_owner" ? "text-primary" : ""} />
              Do'kon egasi
            </button>
          </div>

          <Card className="p-5 sm:p-7 shadow-xl border-border/50">
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <Input placeholder="Ism Familiya" className="pl-10 h-11" value={form.name} onChange={set("name")} required />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <Input placeholder="+998 90 123 45 67" className="pl-10 h-11" value={form.phone} onChange={set("phone")} required />
                </div>
              </div>
              ...
              {role === "shop_owner" && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <Input placeholder="Masalan: Qurilish Asboblari Markazi" className="pl-10 h-11" value={form.shopName} onChange={set("shopName")} />
                  </div>
                </motion.div>
              )}
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <Input type="password" placeholder="Kamida 6 ta belgi" className="pl-10 h-11" value={form.password} onChange={set("password")} required minLength={6} />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" size={16} />
                  <Input type="password" placeholder="Parolni qayta kiriting" className="pl-10 h-11" value={form.confirmPassword} onChange={set("confirmPassword")} required />
                </div>
              </div>

              {/* Terms */}
              <div
                className={`flex items-start gap-3 p-3.5 rounded-xl border-2 transition-colors cursor-pointer ${
                  agreedToTerms ? "border-green-400 bg-green-50/60 dark:bg-green-900/20" : "border-border bg-secondary/40"
                }`}
                onClick={() => setAgreedToTerms(v => !v)}
              >
                <div className={`mt-0.5 w-4.5 h-4.5 rounded flex-shrink-0 flex items-center justify-center border-2 transition-all ${
                  agreedToTerms ? "bg-green-500 border-green-500" : "border-muted-foreground"
                }`}>
                  {agreedToTerms && <Check size={11} strokeWidth={3} color="white" />}
                </div>
                <p className="text-xs leading-relaxed select-none">
                  Men{" "}
                  <button type="button" onClick={e => { e.stopPropagation(); setOfertaOpen(true); }}
                    className="text-primary font-semibold underline underline-offset-2 hover:text-primary/80 inline-flex items-center gap-0.5">
                    ommaviy oferta shartlari <ExternalLink size={11} />
                  </button>
                  {" "}bilan tanishdim va roziman.
                </p>
              </div>

              <Button type="submit" className="w-full h-11 text-base" disabled={registerMutation.isPending || !agreedToTerms}>
                {registerMutation.isPending ? "Ro'yhatdan o'tilmoqda..." : role === "customer" ? "Ro'yhatdan o'tish" : "Do'kon oching"}
              </Button>
            </form>
          </Card>
        </motion.div>
      ...
      <Dialog open={ofertaOpen} onOpenChange={setOfertaOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
          ...
          <div className="overflow-y-auto flex-1 pr-2">
            <pre className="whitespace-pre-wrap text-sm text-muted-foreground font-sans leading-relaxed">{OFERTA_TEXT}</pre>
          </div>
          <div className="pt-4 border-t flex gap-3">
            <Button variant="outline" className="flex-1" onClick={() => setOfertaOpen(false)}>Yopish</Button>
            <Button className="flex-1 gap-2" onClick={() => { setAgreedToTerms(true); setOfertaOpen(false); }}>
              <Check size={16} strokeWidth={2.5} /> Roziman
            </Button>
          </div>
        </DialogContent>
      </Dialog>
